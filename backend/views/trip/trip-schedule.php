   ...
   // $fontColor="#000000";
    ...
    if (date("d",mktime (0,0,0,$month,$d,$year)) == "Sun") {  }
      $today = date("Y-m-d",mktime (0,0,0,$month,$d,$year));
           $trips = $model2->where(['id_company'=>$session['filter']['company'] ])->andWhere(['id_route'=>$session['filter']['route']])->andWhere(['dept_time'=>$session['filter']['time']])->andWhere(['date'=>$today])->orderBy(['dept_time'=>SORT_ASC])->all();
    //tanggal
