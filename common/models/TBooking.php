...
class TBooking extends \yii\db\ActiveRecord
{
    ...
    public function afterSave( $insert, $changedAttributes ){
        ...
        if ($this->id_status == self::STATUS_INVALID || $this->id_status == self::STATUS_EXPIRED ) {
            $data = [
                'id_boat'        => $modelTrip->id_boat,
                'date'           => $modelTrip->date,
                'totalPax'       => count($this->affectedPassengers),
                'changeType'     => $modelTrip::TYPE_UP,
                'dept_time'      => $modelTrip->dept_time,
                'island_route'   => $modelTrip->idRoute->departureHarbor->id_island.'-'.$modelTrip->idRoute->arrivalHarbor->id_island
            ];
