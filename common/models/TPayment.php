...
{
    const METHOD_ON_BOOK              = 0;
    const STATUS_UNCONFIRMED          = 1; //SAME FOR PAYPAL PAYMENT METHOD
    const STATUS_PENDING              = 2; // SAME FOR BCA PAYMENT METHOD
    const STATUS_CONFIRM_NOT_RECEIVED = 3; //SAME FOR PERMATA PAYMENT METHOD
    const STATUS_CONFIRM_RECEIVED     = 4; //SAME FOR MANDIRI BILL PAYMENT
    ...
    {
        return [
            [['email', 'phone', 'total_payment', 'total_payment_idr', 'currency', 'exchange', 'token','id_payment_type','id_payment_method'], 'required'],
            ...
            [['id_payment_method'],'in','range'=>[self::METHOD_ON_BOOK,self::STATUS_UNCONFIRMED,self::STATUS_PENDING, self::STATUS_CONFIRM_NOT_RECEIVED,self::STATUS_CONFIRM_RECEIVED]],
    ...
    { 
        return $this->hasOne(TBankTransferTransaction::className(), ['payment_token' => 'token']); 
    }

    public function afterSave($insert, $changedAttributes){
        if ($this->status == self::STATUS_CONFIRM_NOT_RECEIVED || $this->status == self::STATUS_CONFIRM_RECEIVED) {
            if ($this->id_payment_type == 1) {
              $typeticket = 1; //ticket fastboat;
            }elseif ($this->id_payment_type == 2) {
              $typeticket = 3; //ticket private transfers
            }
            TMailQueue::addTicketQueue($this->id,$typeticket);
        }elseif ($this->status == self::STATUS_EXPIRED || $this->status == self::STATUS_INVALID) {
    ...
    public function setPaymentBookingStatus($payment_status, $booking_status, $log = false, s$event = null){
            $this->status = $payment_status;
            if ($this->id_payment_type == self::PAYMENT_FASTBOAT) {
                $modelBookings = $this->tBookings;
            }elseif ($this->id_payment_type == self::PAYMENT_PRIV_TRANSFERS) {
                $modelBookings = $this->tPrivateBookings;
            }
             if ($log == true) {
                $modelPaymentLog = TPaymentLog::addPaymmentLog($this->id,$event);
                foreach ($modelBookings as $key => $valBooking) {
    ...
    public static function getArrayPaymentByToken($token){
        return TPayment::find()->joinWith(['bankTransfer'])->where(['token'=>$token])->asArray()->one();
    }
    ...
    public static function getVirtualAccount($payment_token){
        $modelPayment = TPayment::find()->where(['token'=>$payment_token])->one();
            if ($modelPayment->id_payment_type == self::PAYMENT_FASTBOAT) {
                foreach ($modelPayment->tBookings as $key => $valueBooking) {
                    $Bookings[] = [
                        'id'       => $valueBooking->id,
                        'price'    => $valueBooking->total_idr,
                        'quantity' => 1,
                        'name'     => 'Ticket Fastboat '.$valueBooking->idTrip->idBoat->name.' - '.$valueBooking->idTrip->idRoute->departureHarbor->name.' To '.$valueBooking->idTrip->idRoute->arrivalHarbor->name.' - '.date('d-m-Y',strtotime($valueBooking->idTrip->date)).' ( '.date('H:i',strtotime($valueBooking->idTrip->dept_time)).' )'
                    ];
                }
            }elseif ($modelPayment->id_payment_type == self::PAYMENT_PRIV_TRANSFERS)  {
                foreach ($modelPayment->tPrivateBookings as $key => $valPrivateBooking) {
                    $Bookings[] = [
                        'id'       => $valPrivateBooking->id,
                        'price'    => $valPrivateBooking->amount_idr,
                        'quantity' => 1,
                        'name'     => 'Ticket Private Transfers : '.$valPrivateBooking->idTrip->idRoute->fromRoute->location.' To '.$valPrivateBooking->idTrip->idRoute->toRoute->location.' - '.date('d-m-Y',strtotime($valPrivateBooking->date_trip)).' ( '.date('H:i',strtotime($valPrivateBooking->date_trip)).' )'
                    ];
                }
            }
        ...
        $baseArray = [
            'transaction_details' => [
                'gross_amount' => $modelPayment->total_payment_idr,
                'order_id'     => $modelPayment->token
            ],
            'customer_details'    => [
                'first_name'   => $modelPayment->name,
                'email'        => $modelPayment->email,
                'phone'        => $modelPayment->phone
            ],
            'item_details'        => $Bookings,
            
            'custom_expiry'    => [
                //'order_time'      => date('Y-m-d H:i:s').' +0800',//'2016-12-07 11:54:12 +0700' 
                'expiry_duration' => 120,
                'unit'            => 'minute'
            ]

        ];
        $paymentMethod = $modelPayment->id_payment_method;
        //BCA ATAU PERMATA VA
        if ($paymentMethod == 2 || $paymentMethod == 3) {
            $bankTransfersArray = [
                'payment_type'        => 'bank_transfer',
                'bank_transfer'       => [
                    'bank'         => $modelPayment->idPaymentMethod->method
                ],
            ];
        $finalArray = array_merge($baseArray, $bankTransfersArray);
        //MANDIRI BILL PAYMENT
        }elseif ($paymentMethod == 4) {
            $echannelArray = [
                'payment_type' => 'echannel',
                'echannel' => [
                    'bill_info1'   => 'Payment Gilitransfers for :',
                    'bill_info2'   => $modelPayment->idPaymentType->type
                ]
            ];
        $finalArray = array_merge($baseArray, $echannelArray);
        }

          $curl = curl_init();
          curl_setopt_array($curl, array(
          CURLOPT_URL            => Yii::$app->params['MidtransVaUrl'],
          ...
          CURLOPT_POSTFIELDS     => Json::encode($finalArray),
          CURLOPT_HTTPHEADER     => array(
                "Accept: application/json",
                "Authorization: Basic ".Yii::$app->params['MidtransEncodeUser']."",
